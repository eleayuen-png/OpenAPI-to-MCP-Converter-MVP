      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-slate-600 dark:text-slate-400 max-w-2xl">
            Chain multiple endpoints into single agentic tools to save context tokens and prevent hallucinations.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest px-1">Your Macros</h3>
            {macros.length === 0 ? (
          ...
          <div className="lg:col-span-2">
            {activeMacroId ? (
                ...
                {macros.filter((m: any) => m.id === activeMacroId).map((macro: any) => (
                  <div key={macro.id} className="space-y-6">
                    <div>
                      ...
                      />
                      <label className="block text-xs font-bold text-slate-400 uppercase mt-4 mb-2">Description</label>
                      <textarea 
                    ...
                    <div className="space-y-3">
                      <label className="block text-xs font-bold text-slate-400 uppercase">Sequence</label>
                      {macro.steps.length === 0 ? (
                        <div className="p-8 border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-xl text-center text-slate-400 text-sm">
                          Add steps from your active endpoints.
                        </div>
                      ) : (
                    ...
                    <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
                      <label className="block text-xs font-bold text-slate-400 uppercase mb-3">Available Endpoints</label>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto custom-scrollbar">
                        {availableEndpoints.map((ep: any) => (
                          ...
                          >
                            ...
                            <span className="font-bold text-blue-600 uppercase">{ep.method}</span>
              ...
              <div className="h-full min-h-[400px] flex flex-col items-center justify-center p-12 bg-slate-50/50 dark:bg-slate-900/20 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl text-slate-400">
                ...
                <p>Select or create a macro to begin bundling</p>
              </div>
            )}
          </div>
        </div>
