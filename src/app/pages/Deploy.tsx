...
const PLANS = [
  ...
  { id: 'semiannual', label: 'Semi-Annual', price: '$109', billingNote: 'every 6 months',  perMonth: '~$18.17 / mo',   savingsVsMonthly: 'Save $5', link: () => import.meta.env.VITE_STRIPE_PAYMENT_LINK_SEMIANNUAL },
  { id: 'annual',     label: 'Annual',      price: '$219', billingNote: 'per year',         perMonth: '~$18.25 / mo',   savingsVsMonthly: 'Save $9', link: () => import.meta.env.VITE_STRIPE_PAYMENT_LINK_ANNUAL },
] as const;
      ...
      <div className="bg-white dark:bg-[#111827] w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-300">
        ...
        <div className="relative bg-gradient-to-br from-blue-600 to-indigo-800 p-8 text-center overflow-hidden">
          <div className="absolute top-4 right-4 z-10">
            <button onClick={() => { setShowPlans(false); onClose(); }} className="p-2 bg-black/20 hover:bg-black/40 rounded-full text-white/80 hover:text-white transition-colors">
              <X className="h-5 w-5" />
            </button>
          </div>
          {showPlans && (
            <button onClick={() => setShowPlans(false)} className="absolute top-4 left-4 z-10 p-2 bg-black/20 hover:bg-black/40 rounded-full text-white/80 hover:text-white transition-colors">
          ...
          <h2 className="relative z-10 text-2xl font-bold text-white mb-2 tracking-tight">
            {user?.isAnonymous ? "Sign in to unlock Pro features" : showPlans ? "Choose your plan" : `Unlock ${featureName}`}
          </h2>
          <p className="relative z-10 text-blue-100 text-sm max-w-xs mx-auto">
            {user?.isAnonymous ? "Create an account to save your config and access enterprise tools."
              : showPlans ? "All plans include every Pro feature. Cancel anytime."
        ...
        <div className="p-8">
          {!showPlans ? (
            <>
              ...
              <p className="text-center text-xs text-slate-400 mt-4">Secure checkout via Stripe · Cancel anytime</p>
            </>
          ) : (
            <div className="grid grid-cols-2 gap-3">
              {PLANS.map((plan) => (
                ...
                >
                  ...
                  <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">{plan.label}</span>
                  <span className="text-2xl font-bold text-[#141B41] dark:text-white">{plan.price}</span>
                  <span className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{plan.billingNote}</span>
                  <span className="text-xs text-slate-400 dark:text-slate-500 mt-1">{plan.perMonth}</span>
    ...
    <div className="bg-white dark:bg-[#111827] rounded-2xl border border-slate-200 dark:border-slate-800 p-8 sm:p-12 text-center transition-colors shadow-sm flex flex-col items-center">
      ...
      <div className="min-h-[40px] flex items-center justify-center mb-8">
         <p className="text-slate-500 dark:text-slate-400 max-w-lg mx-auto leading-relaxed text-center">
            You have selected <span className="text-blue-600 dark:text-blue-400 font-bold">{selectedEndpoints?.size || 0}</span> endpoints. We will securely deploy an MCP gateway instance for your agent.
          </p>
      </div>
      ...
      <div className="w-full max-w-md mb-8 space-y-6 text-left">
        <div>
          ...
          <input 
            ...
            className="w-full bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3 text-[#141B41] dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all" 
          />
        </div>
      ...
      <button 
        onClick={onDeploy} 
        disabled={isDeploying || !selectedEndpoints || selectedEndpoints.size === 0 || !baseUrl.trim()} 
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-8 animate-in fade-in duration-700">
      ...
      {!deploymentInfo ? (
        ...
        <DeploymentSuccess info={deploymentInfo} count={selectedEndpoints?.size || 0} />
      )}
    </div>
  );
}
